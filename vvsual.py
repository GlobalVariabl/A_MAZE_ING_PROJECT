    ...
    try:
        ...
        for y in range(height):
            ...
            row_bottom = (wall_color +
                          (wall_type["corner_bl"] if y == height - 1 else wall_type["v"]) + COLORS["reset"])
            for x in range(width):
                ...
                if (y, x) == start:
                    cell_char = (COLORS["reset"] + solve_color + " S " +
                                 COLORS["reset"] + wall_color)
                elif (y, x) == end:
                    ...
                    if (y, x, 0) in path_solve:
                        cell_char = (COLORS["reset"] + "\033[41m" + " 1 " +
                                     COLORS["reset"] + wall_color)
                    elif (y, x, 1) in path_solve:
                        cell_char = (COLORS["reset"] + "\033[42m" + " 2 " +
                                     COLORS["reset"] + wall_color)
                    elif (y, x, 2) in path_solve:
                        cell_char = (COLORS["reset"] + "\033[43m" + " 3 " +
                                     COLORS["reset"] + wall_color)
                    else:
                        cell_char = "   " + wall_color
                elif all(cell_walls.values()):
                    cell_char = (COLORS["reset"] + space_color + "░░░" +
                                 COLORS["reset"] + wall_color)
                else:
                    cell_char = "   " + wall_color

                row_top += (wall_color + cell_char +
                            (wall_type["v"] if cell_walls['E'] or x == width - 1 else " ") + COLORS["reset"])
                ...
                if x < width - 1:
                    ...
                    if y < height - 1:
                        row_bottom += (COLORS["reset"] + wall_color + "┃" +
                                       COLORS["reset"])
                    else:
